...
export default function UserAgentTool() {
  ...
  const content =
    useToolContent<UserAgentToolContent>("user-agent");
    ...
    <div className="grid gap-8">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <InfoCard
          ...
          value={`${info.os.name || "Unknown"} ${
            info.os.version || ""
          }`}
          sub={
            info.cpu.architecture
              ? `${content.ui.cards.os.archPrefix} ${
                  info.cpu.architecture
                }`
              : ""
          }
        ...
        <InfoCard
          ...
          value={
            info.cpu.architecture ||
            content.ui.cards.cpu.fallback
          }
          sub={
            cores
              ? `${cores} ${content.ui.cards.cpu.coresSuffix}`
              : ""
          }
          theme={theme}
      ...
      >
        <h3
          className={`font-bold mb-4 flex items-center gap-2 ${theme.text}`}
        >
...
function InfoCard({
  ...
  return (
    ...
    >
      ...
      <h4
        className={`text-xs font-bold opacity-60 ${theme.textMuted}`}
      >
        {title}
      </h4>
...
function DetailItem({ label, value, theme }: any) {
  return (
    <div className="flex justify-between items-center border-b border-dashed pb-2 last:border-0 border-zinc-200 dark:border-zinc-800">
      ...
      <span
        className={`font-mono font-bold dir-ltr ${theme.text}`}
      >
        {value}
      </span>
    </div>
