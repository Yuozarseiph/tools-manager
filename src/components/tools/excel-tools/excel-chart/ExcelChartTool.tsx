...
export default function ExcelChartTool() {
  ...
  return (
    <div className={`rounded-2xl border overflow-hidden transition-all duration-300 ${theme.card} ${theme.border} shadow-sm`}>
      ...
      <div className={`p-6 border-b flex flex-col gap-6 ${theme.border}`}>
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
          <label className="flex items-center justify-center gap-2 px-5 py-2.5 bg-blue-600 text-white font-medium rounded-xl cursor-pointer hover:bg-blue-700 transition-all shadow-lg shadow-blue-600/20 w-full sm:w-auto active:scale-95">
            <UploadCloud size={20} />
            <span>{data.length > 0 ? "تغییر فایل اکسل" : "آپلود فایل اکسل"}</span>
            ...
            <div className="flex gap-1 bg-gray-100 dark:bg-gray-800 p-1.5 rounded-xl border border-transparent dark:border-white/5">
              {[
                ...
                <button
                  ...
                  title={type.label}
                >
                  <type.icon size={20} strokeWidth={chartType === type.id ? 2.5 : 2} />
                </button>
              ))}
            </div>
          )}
        ...
        {data.length > 0 && (
          <div className={`p-4 rounded-xl border ${theme.border} bg-gray-50/30 dark:bg-white/5 flex flex-col gap-4`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-sm font-bold opacity-70">
                <Settings2 size={16} />
                <span>تنظیمات نمایش داده‌ها</span>
              </div>
              <span className="text-xs opacity-50">
                نمایش {filteredData.length} رکورد از {data.length}
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="flex flex-col gap-2">
                <label className="text-xs opacity-70">
                  محدوده داده‌ها{" "}
                  {chartType === "pie" && (
                    <span className="text-amber-500 font-bold">(پیشنهاد: حداکثر ۲۰ آیتم)</span>
                  )}
                </label>
                <div className="flex items-center gap-2">
                  <input
                    type="number" min="0" max={data.length}
                    value={startIndex} onChange={(e) => setStartIndex(Number(e.target.value))}
                    className={`w-20 p-1.5 text-center rounded-lg border ${theme.border} bg-transparent`}
                  />
                  <span className="opacity-50">-</span>
                  <input
                    type="number" min="0" max={data.length}
                    value={endIndex} onChange={(e) => setEndIndex(Number(e.target.value))}
                    className={`w-20 p-1.5 text-center rounded-lg border ${theme.border} bg-transparent`}
                  />
                </div>
              </div>
              ...
              {chartType !== "pie" ? (
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between text-xs opacity-70">
                    <span className="flex items-center gap-1"><Minimize2 size={12} /> فشرده</span>
                    <span className="flex items-center gap-1"><Maximize2 size={12} /> گسترده (اسکرول)</span>
                  </div>
                  <input
                ...
                <div className="flex items-center justify-center text-xs text-gray-400 border rounded-lg border-dashed">
                  اسکرول برای نمودار دایره‌ای غیرفعال است
                </div>
              )}
            </div>
          </div>
      ...
      <div className={`bg-white dark:bg-gray-950 min-h-[450px] flex flex-col items-center justify-center p-4 sm:p-8 transition-colors duration-300 relative`} dir="ltr">
        {data.length > 0 ? (
          ...
          <div className="text-center opacity-40 flex flex-col items-center animate-pulse">
            <BarChart3 size={80} className="mb-6 text-gray-300 dark:text-gray-700" strokeWidth={1} />
            <h4 className={`text-lg font-bold mb-2 ${theme.text}`}>نموداری موجود نیست</h4>
            <p className={`text-sm ${theme.textMuted}`}>برای مشاهده نمودار، ابتدا یک فایل اکسل آپلود کنید.</p>
          </div>
        )}
      </div>
    </div>
