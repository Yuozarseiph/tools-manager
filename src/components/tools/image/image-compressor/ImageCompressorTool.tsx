...
export default function ImageCompressorTool() {
  ...
  const content: ImageCompressorToolContent = useImageCompressorContent();
  ...
  const originalSizeKB = file && (file.size / 1024).toFixed(1);
  const compressedSizeKB =
    compressedFile && (compressedFile.size / 1024).toFixed(1);
        ...
        >
                ...
                <div className="flex gap-3 text-xs mt-1">
                  {originalSizeKB && (
                    <span className={theme.textMuted}>
                      {content.ui.fileInfo.original} {originalSizeKB} KB
                    </span>
                  )}
          ...
          <div className={`p-6 rounded-2xl border ${theme.border} ${theme.bg}`}>
            <div className="flex items-center gap-2 mb-4 font-bold">
              <Sliders size={18} /> {content.ui.settings.title}
            </div>
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span>{content.ui.settings.qualityLabel}</span>
          ...
          <div className="flex gap-4">
            ...
            >
              <Download /> {content.ui.buttons.download}
            </button>
          </div>
        </motion.div>
      )}
