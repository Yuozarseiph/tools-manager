...
export default function TextToPdfTool() {
  ...
  const content =
    useToolContent<TextToPdfToolContent>("text-to-pdf");
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-6">
      ...
      >
        <div className="flex items-center gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            >
              {[8, 10, 12, 14, 16, 18, 20, 24, 28, 32].map(
                (size) => (
                  <option
                    key={size}
                    value={size}
          ...
          <div
            className={`w-px h-6 ${theme.border} border-r`}
          />
      ...
      {/* راهنما */}
      <div
        className={`p-4 rounded-xl border ${theme.card} ${theme.border}`}
      >
        <h3
          className={`font-bold mb-2 flex items-center gap-2 ${theme.text}`}
        >
          <FileText size={16} />
          {content.ui.guide.title}
        </h3>
        <ul
          className={`text-sm space-y-1 ${theme.textMuted} mr-6 list-disc`}
        >
