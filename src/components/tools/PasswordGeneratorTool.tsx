...
import { Copy, RefreshCw, Check, ShieldCheck, ShieldAlert, Eye, EyeOff } from 'lucide-react';
...
export default function PasswordGeneratorTool() {
  ...
  const currentStrength = mode === 'generate' ? calculateStrength(password) : calculateStrength(userPass);
  ...
  return (
    <div className={`max-w-2xl mx-auto rounded-3xl border p-6 md:p-8 shadow-xl ${theme.card} ${theme.border}`}>
      
      {/* Tabs */}
      <div className="flex p-1 mb-8 bg-zinc-100 dark:bg-zinc-800 rounded-xl">
        <button 
          onClick={() => setMode('generate')}
          className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${mode === 'generate' ? 'bg-white dark:bg-zinc-700 shadow-sm text-blue-600' : 'text-zinc-400'}`}
        >
          تولید رمز عبور
        </button>
        <button 
          onClick={() => setMode('analyze')}
          className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${mode === 'analyze' ? 'bg-white dark:bg-zinc-700 shadow-sm text-blue-600' : 'text-zinc-400'}`}
        >
          سنجش قدرت
        </button>
      </div>
      ...
      {mode === 'generate' ? (
        <>
          ...
          <div className={`relative flex items-center justify-between p-6 rounded-2xl border-2 mb-8 transition-all ${theme.bg} ${copied ? 'border-green-500' : theme.border}`}>
            ...
            <div className={`absolute bottom-0 left-0 h-1 transition-all duration-500 ${getStrengthColor(currentStrength)}`} style={{ width: `${(currentStrength / 5) * 100}%` }} />
          </div>
        ...
        <>
          {/* Analyze UI */}
          <div className="space-y-6">
            <div className={`relative p-1 rounded-2xl border-2 transition-all focus-within:border-blue-500 ${theme.bg} ${theme.border}`}>
              <input
                type={showPass ? "text" : "password"}
                value={userPass}
                onChange={(e) => setUserPass(e.target.value)}
                placeholder="رمز خود را اینجا وارد کنید..."
            ...
            {userPass && (
              <div className={`p-6 rounded-2xl border text-center ${theme.card} ${theme.border}`}>
                <div className="flex justify-center mb-4">
                  {currentStrength <= 2 ? <ShieldAlert size={48} className="text-red-500" /> : <ShieldCheck size={48} className={currentStrength >= 5 ? "text-green-500" : "text-yellow-500"} />}
                </div>
                <h3 className={`text-2xl font-black mb-2 ${getStrengthColor(currentStrength).replace('bg-', 'text-')}`}>
                  {getStrengthLabel(currentStrength)}
                </h3>
                <p className={theme.textMuted}>
                  {currentStrength <= 2 ? 'این رمز خیلی زود هک می‌شود!' : currentStrength <= 4 ? 'خوب است، اما می‌تواند بهتر باشد.' : 'عالی! هک کردن این رمز سال‌ها طول می‌کشد.'}
                </p>
              </div>
            )}
          </div>
