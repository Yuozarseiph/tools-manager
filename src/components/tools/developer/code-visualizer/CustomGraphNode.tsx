...
const getIconForType = (type: string, data: CustomNodeData) => {
  ...
  if (type === "annotation") return Award;
  if (type === "synchronized") return Lock;
  ...
  if (type === "function" || type === "method") {
    ...
    if (data.isSynchronized) return Lock;
    if (data.isFinal) return CheckCircle;
    if (data.isStatic) return Zap;
    return PlayCircle;
  }
...
const getNodeStyle = (type: string, data: CustomNodeData, theme: any) => {
  ...
  const getBorder = () => {
    const baseBorders: Record<string, string> = {
      ...
      package: isDark ? "border-indigo-600" : "border-indigo-400",
      import: isDark ? "border-sky-600" : "border-sky-400",
      annotation: isDark ? "border-yellow-600" : "border-yellow-400",
      synchronized: isDark ? "border-purple-600" : "border-purple-400",
    };

    let border =
      baseBorders[type] || (isDark ? "border-gray-700" : "border-gray-300");

    if (data.isAbstract) border += " border-dashed";
    if (data.isVirtual) border += " border-dotted";
    if (data.isFinal) border += " border-2";
  ...
  const getBackground = () => {
    const baseBackgrounds: Record<string, string> = {
      ...
      package: isDark ? "bg-indigo-900/20" : "bg-indigo-50/70",
      import: isDark ? "bg-sky-900/20" : "bg-sky-50/70",
      annotation: isDark ? "bg-yellow-900/20" : "bg-yellow-50/70",
      synchronized: isDark ? "bg-purple-900/20" : "bg-purple-50/70",
    };
  ...
  const getIconColor = () => {
    const iconColors: Record<string, string> = {
      ...
      package: isDark ? "text-indigo-400" : "text-indigo-600",
      import: isDark ? "text-sky-400" : "text-sky-600",
      annotation: isDark ? "text-yellow-400" : "text-yellow-600",
      synchronized: isDark ? "text-purple-400" : "text-purple-600",
    };
...
const CustomGraphNode = ({ data, selected }: CustomNodeProps) => {
  ...
  const getAccessLevelColor = (level: string) => {
    if (isDark) {
      if (level === "public") return "bg-green-900/40 text-green-300";
      if (level === "private") return "bg-red-900/40 text-red-300";
      if (level === "protected") return "bg-yellow-900/40 text-yellow-300";
      return "bg-gray-900/40 text-gray-300";
    }
    if (level === "public") return "bg-green-100 text-green-800";
    if (level === "private") return "bg-red-100 text-red-800";
    if (level === "protected") return "bg-yellow-100 text-yellow-800";
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3">
        <div
          className={`mt-0.5 p-2 rounded-lg ${isDark ? "bg-white/10" : "bg-black/5"} ${styles.icon}`}
        >
        ...
        <div className="flex flex-col flex-1 min-w-0">
          <div className="flex items-center justify-between mb-2 gap-2">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {accessLevel && (
                <span
                  className={`text-xs px-2 py-1 rounded ${getAccessLevelColor(accessLevel)}`}
                >
                  {accessLevel}
                </span>
              )}
              ...
              {returnType && (
                <span
                  className={`text-xs font-mono ${theme.textMuted} hidden sm:inline`}
                >
                  → {returnType}
                </span>
              )}
