...
export default function IPCheckerTool() {
  ...
  const content =
    useToolContent<IPCheckerToolContent>("ip-checker");
      ...
      >
        ...
        {loading ? (
          <div className="flex flex-col items-center gap-4 py-10">
            ...
            <p
              className={`text-sm animate-pulse ${theme.textMuted}`}
            >
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        {flag && (
          <img
            src={`https://flagcdn.com/w40/${String(
              sub
            ).toLowerCase()}.png`}
            alt="flag"
        ...
        <div
          className={`text-lg font-bold truncate ${theme.text}`}
        >
          {value}
        </div>
      </div>
      <div
        className={`text-xs font-mono mt-1 opacity-60 ${theme.text}`}
      >
        {sub}
      </div>
    </div>
