...
import { DOCS_BY_LOCALE, type DocSectionItem } from "@/data/docs";
...
export default function DocsPage() {
  ...
  const docs: DocSectionItem[] = DOCS_BY_LOCALE[locale];

  const [selectedTool, setSelectedTool] = useState<string>(docs[0]?.id);
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);

  useEffect(() => {
    if (docs[0]) setSelectedTool(docs[0].id);
  }, [locale]); 

  const categories = Array.from(new Set(docs.map((doc) => doc.category)));
  const currentDoc =
    docs.find((d) => d.id === selectedTool) ?? docs[0];
