// app/tools/(image)/image-stitcher/ImageStitcher.tsx
"use client";

import Link from "next/link";
import { ArrowRight, StretchVertical } from "lucide-react";

import { useThemeColors } from "@/hooks/useThemeColors";
import ImageStitcherTool from "@/components/tools/image/image-stitcher/ImageStitcherTool";
import { useImageStitcherPageContent } from "./content";
import { useLanguage } from "@/context/LanguageContext";

export default function ImageStitcherClient() {
  const theme = useThemeColors();
  const { locale } = useLanguage();
  const page = useImageStitcherPageContent();
      ...
      <div className="max-w-5xl mx-auto px-6 pt-10 w-full">
        ...
        <div className="flex items-center gap-4 mb-2">
          <div className={`p-3 rounded-xl ${theme.primary}`}>
            <StretchVertical size={24} className="text-white" />
          </div>
